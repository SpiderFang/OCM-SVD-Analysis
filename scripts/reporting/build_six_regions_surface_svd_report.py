...
def add_summary_chapters(document: Document, data: Sequence[dict[str, object]], counters: dict[str, int]) -> None:
    ...
    document.add_paragraph(
        "月份檔案合併後先依 UTC 排序，重複時間戳採 prefer-last 去重。為維持共用快取的內部時間一致性，貢寮與龜山島對"
        "2025 年 7 月首 24 筆時間標籤採用專案端預先登錄的 +24 小時時間軸正規化假設；此處置依快取跨月時序關係制定，"
        "並非原始 NetCDF 資料提供者的更正或確認，且只改變分析記憶體中的時間座標，不改寫 u、v 或 η 值；凡涉及該月絕對"
        "時刻的解讀均以此假設為條件。貢寮與龜山島最終"
...
def add_focused_summary_chapters(document: Document) -> None:
    ...
    document.add_paragraph(
        ...
        "2025 年 7 月首 24 筆時間標籤採用專案端預先登錄的 +24 小時時間軸正規化假設；此處置依快取跨月時序關係制定，"
        "並非原始 NetCDF 資料提供者的更正或確認，且只改變分析記憶體中的時間座標，不改寫 u、v 或 η 值；凡涉及該月絕對"
        "時刻的解讀均以此假設為條件。所有 PC 圖均保留"
        "資料缺口，不跨缺口插值；時序中的空白或不連續區段代表"
        "來源資料未提供樣本。"
    )
